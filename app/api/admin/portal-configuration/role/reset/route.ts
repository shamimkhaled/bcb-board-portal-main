...
export async function POST(request: NextRequest) {
  ...
  try {
    const role = validateRole(await request.json());
    const previousValue = await snapshotRolePortalConfiguration(role);
    await prisma.$transaction((client) => resetRolePortalConfiguration(client, role));
    const newValue = await snapshotRolePortalConfiguration(role);
    const meta = getRequestMeta(request);

    await auditPortalConfigurationChange({
      auth,
      actionType: "ADMIN_PORTAL_ROLE_PRESET_RESET",
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to reset role preset" }, { status: 400 });
  }
}
